...
D3D::~D3D()
{
	...
	if (m_depthLessEqualStencilState)
	{
		m_depthLessEqualStencilState->Release();
		m_depthLessEqualStencilState = 0;
	}
						  ...
						  float screenDepth, float screenNear)
{
	...
	result = D3D11CreateDeviceAndSwapChain(adapter, D3D_DRIVER_TYPE_UNKNOWN, NULL, D3D11_CREATE_DEVICE_BGRA_SUPPORT, &featureLevel, 1, 
	...
	result = m_device->CreateDepthStencilState(&depthStencilDesc, &m_depthLessEqualStencilState);
	if (FAILED(result))
	{
		return false;
	}
...
void D3D::SetDepthLess()
{
	m_deviceContext->OMSetDepthStencilState(m_depthStencilState, 1);
}

void D3D::SetDepthLessEqual()
{
	m_deviceContext->OMSetDepthStencilState(m_depthLessEqualStencilState, 1);
}
